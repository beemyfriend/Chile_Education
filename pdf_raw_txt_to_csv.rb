#Comunas is a hash with Comuna as a key and circunscripcion as the values
#if a comuna is not included in the hash, then the only circunscripcion in that 
#comuna is the comuna
...
#the string_to_csv method parses a string (a line from the parsed pdf) and converts the 
#information into a semi-colon seperated variable. 
...
#The identify_comuna_and_header method takes a text file (a fully converted pdf to text page) and
#identifies the comuna and the last line of header information. The last line is to identify the 
#first line in the document that can be converted into csv format 
...
#The parse_line_for_csv_record parses each line with a text file (pdf page converted to text)
#sometimes the pdf to text version converts a single line from the pdf to multiple lines with 
#odd spacing in the text conversion

#start_line should be lasthead + 1
